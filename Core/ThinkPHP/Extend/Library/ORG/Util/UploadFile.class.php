...
class UploadFile {//类定义开始
    ...
    private function save($file) {
		...
		if(!empty($this->water)){
            ...
            if(false!==$ret){
                if($this->water_savename){
                    $this -> file_upload($tmp_water,$this->water_savename);
                }else{
		...
		//[cluster] 上传文件
        if(!$this->thumbRemoveOrigin && !$this -> file_upload($uploadfile, $this->autoCharset($filename,'utf-8','gbk'))) {
            $this->error = '文件上传保存错误！';
            return false;
        }
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
					...
					if(false!==$ret) $this -> file_upload($tmp_file,$thumbPath.$thumbname.'.'.$thumbExt);
